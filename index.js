...
const CONFIG = {
  ...
  RETRY_DELAYS: [3000, 8000, 20000],
  ERR_COOLDOWN: 10 * 60 * 1000,
};
...
const JMA_DICT = {
  ...
  '震源地': '진원지', '陸地': '육지', '海域': '해역', '沿岸': '연안',
  '予想': '예상', '到達': '도달', '高い': '높음', '低い': '낮음',
  '強い': '강함', '弱い': '약함', '危険': '위험', '安全': '안전',
...
async function translateToKo(text) {
  ...
  if (!/[\u3040-\u30FF\u4E00-\u9FFF]/.test(preTranslated)) {
    xlCache.set(cacheKey, preTranslated);
    return preTranslated;
  }

  let result = preTranslated;
  try { result = await translateGoogle(preTranslated); }
  catch {
    try { result = await translateMyMemory(preTranslated); }
    catch { /* 로컬 치환 결과 사용 */ }
  }

  if (xlCache.size > 500) xlCache.delete(xlCache.keys().next().value);
...
async function fetchNDMS() {
  ...
  try {
    const items = await fetchWithRetry(async () => {
      ...
      if (Array.isArray(body)) return body[0]?.data ?? body[0]?.items ?? body[0]?.item ?? body ?? [];
    ...
    for (const e of list) {
      ...
      const type      = e.DSSTR_SE_NM ?? e.dstrSeNm ?? '알림';
      const content   = e.MSG_CN      ?? e.msgCn    ?? '';
      const area      = e.RCV_AREA_NM ?? e.rcvAreaNm ?? '전국';
      const createdAt = (e.CRT_DT     ?? e.crtDt    ?? '').replace(/\//g, '-');
...
async function fetchKMA() {
  ...
  try {
    ...
    for (const e of itemList) {
      ...
      if (e.mtSt) fields.push({ name: '📊 최대 진도', value: String(e.mtSt), inline: true });
...
function parseJMAContent(content) {
  ...
  return {
    mag:   content.match(/M\s*([\d.]+)/)?.[1]            ? Number(content.match(/M\s*([\d.]+)/)[1])   : null,
    depth: (content.match(/深さ.*?([\d]+)\s*km/)?.[1] ?? content.match(/깊이.*?([\d]+)\s*km/)?.[1])   ? Number(content.match(/深さ.*?([\d]+)\s*km/)?.[1] ?? content.match(/깊이.*?([\d]+)\s*km/)?.[1]) : null,
    lat:   content.match(/北緯\s*([\d.]+)/)?.[1]          ? Number(content.match(/北緯\s*([\d.]+)/)[1]) : null,
    lon:   content.match(/東経\s*([\d.]+)/)?.[1]          ? Number(content.match(/東経\s*([\d.]+)/)[1]) : null,
  };
}
...
async function fetchJMA() {
  try {
    const items = await fetchWithRetry(async () => {
      ...
      const entries = parser.parse(res.data)?.feed?.entry;
    ...
    for (const e of items) {
      ...
      const color     = isTsunami ? 0x0000FF : isVolcano ? 0xFF4500 : parsed.mag ? magMeta(parsed.mag).color : 0x5865F2;
      ...
      fields.push({ name: '🗺️ 지도', value: `[구글 지도 보기](${createMapLink(parsed.lat, parsed.lon, titleKo)})`, inline: true });
...
client.on('interactionCreate', async (i) => {
  ...
  if (i.commandName === '청소' && OWNER_ID && i.user.id !== OWNER_ID)
    ...
    return i.reply({ embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🔁 조회 주기',     value: '안전안내문자: 2분 / 지진: 5분', inline: false },
        )
        .setTimestamp()
    ]});
  }
  ...
  if (i.commandName === '청소') {
    ...
    return i.reply(
      `🧹 캐시가 초기화되었습니다.\n삭제 — NDMS: ${counts.ndms}개 / KMA: ${counts.kma}개 / JMA: ${counts.jma}개`
    );
  }
  ...
  if (i.commandName === '도움말') {
    return i.reply({ embeds: [
      new EmbedBuilder()
        ...
        .setColor(0x5865F2)
        .setDescription('국내외 지진, 화산, 쓰나미, 재난문자를 실시간으로 알려드립니다.')
        .addFields(
          { name: '📢 안전안내문자 (NDMS)', value: '행정안전부 안전안내문자를 **2분**마다 확인합니다.', inline: false },
          { name: '🌏 국내 지진 (KMA)',     value: '기상청 지진 정보를 **5분**마다 확인합니다.',        inline: false },
          { name: '🗾 일본 지진/화산 (JMA)', value: '일본 기상청 정보를 **5분**마다 확인합니다. (자동 번역)', inline: false },
          { name: '⚙️ 커맨드',
            value: '`/상태` `/마지막` `/청소` (Owner) `/도움말`',
            inline: false },
          { name: '🎨 규모별 색상',
            value: '⚪ M3미만  🔵 M3+  🟡 M4+  🟠 M5+  🔴 M6+  🆘 M7+',
            inline: false },
        )
        .setFooter({ text: '오류 발생 시 채널에 자동 알림 (동일 오류 10분 쿨다운)' })
        .setTimestamp()
    ]});
  }
});
...
/* =========================
   9. 웹 서버 (Express v5 호환)
   ✅ v5에서 app.listen()은 Promise를 반환하므로 체이닝 불가 → 분리
========================= */
const app = express();

app.get('/', (req, res) => {
  res.json({ status: 'running', uptime: Math.floor(process.uptime()), stats, lastFetch });
});
