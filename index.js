/**
 * ╔═══════════════════════════════════════════════════════════════╗
 * ║  재난 알림 봇 v9.7  —  Ultimate Stable Edition                ║
 * ║  소스: KMA · JMA · NDMS · USGS                               ║
 * ║                                                              ║
 * ║ [30분 심층 최적화 및 무결점 스펙]                              ║
 * ║  ├─ 네트워크 완전 단절(Timeout/DNS) 자동 복구 백오프 탑재    ║
 * ║  ├─ Discord 메모리 누수 원천 차단 (백그라운드 GC 데몬 가동)  ║
 * ║  ├─ USGS 타임 싱크 400 에러 및 중복 수신 방어망 고도화       ║
 * ║  ├─ AI 무료 최고성능 병렬망 (Gemini 2.0 / Llama 3.3 70B)     ║
 * ║  ├─ HTTP Agent 소켓 폭주(EMFILE) 및 좀비 커넥션 차단         ║
 * ║  ├─ 병렬 Abort 중 발생하는 Axios Cancel 쓰레기 로그 은닉     ║
 * ║  └─ Uncaught Exception 격리망 구성 (봇 절대 안 꺼짐)         ║
 * ╚═══════════════════════════════════════════════════════════════╝
 */
...
const ENV = (() => {
  if (!process.env.DISCORD_TOKEN) {
    process.stdout.write('[FATAL] DISCORD_TOKEN 환경변수가 없습니다.\n');
    process.exit(1);
  }
...
const CFG = Object.freeze({
  ...
  SENT_MAX:      2_000,  // 파일 영속화 메모리 최대 한도
  XL_MAX:        800,    // 번역 캐시 최대 한도
  ...
  RAID_CH_LIMIT:       5,
  RAID_CH_SEC:         10,
  BULK_DELETE_MS:      14 * 24 * 60 * 60 * 1000,
...
/* ══════════════════════════════════════════════════════════════
   §3. HTTP Client (소켓 고갈 방지 및 Keep-Alive 최적화)
══════════════════════════════════════════════════════════════ */
const HTTP = axios.create({
  timeout: 15_000,
  headers: { 'User-Agent': 'DisasterBot/9.7', 'Accept': 'application/json, application/xml, */*', 'Accept-Language': 'ko-KR,ko;q=0.9' },
  // 🚀 maxSockets 설정으로 대규모 동시 요청 시 포트/소켓 고갈(EMFILE 에러) 완벽 차단
  httpsAgent: new https.Agent({ keepAlive:true, maxSockets: 50, maxFreeSockets: 10, rejectUnauthorized:false }),
...
/* ══════════════════════════════════════════════════════════════
   §4. AI 예측 통합망 (비동기 지연 복구 / Promise.any 최적화)
══════════════════════════════════════════════════════════════ */
// 🚀 통신 두절(Timeout, DNS에러) 및 429/500 에러를 모두 방어하는 극한의 백오프 래퍼
async function aiFetchWithRetry(url, payload, headers, signal) {
  let lastErr;
  for (let i = 0; i < 3; i++) {
    try {
      return await axios.post(url, payload, { headers, timeout: 15_000, signal });
    } catch (e) {
      lastErr = e;
      if (axios.isCancel(e) || e.name === 'CanceledError' || e.code === 'ERR_CANCELED') throw e; // Abort 시 즉각 반환
      
      const status = e.response?.status;
      // 응답이 없거나(네트워크 단절), 429 제한이거나, 서버 500 에러일 경우 재시도
      const isRecoverable = !e.response || status === 429 || status >= 500;
      
      if (isRecoverable && i < 2) {
        const delay = Math.floor(Math.pow(2, i) * 1500 + Math.random() * 500); // 1.5s -> 3.5s + Jitter
        await sleep(delay);
        continue;
      }
      throw e;
...
const AI_PROVIDERS =[
  {
    id: 'gemini', name: 'Gemini 2.0 Flash (Google)', key: () => ENV.GEMINI_API_KEY,
    call: async (prompt, system, key, signal) => {
      ...
      const res = await aiFetchWithRetry(url, payload, { 'Content-Type': 'application/json' }, signal);
      return res.data?.candidates?.[0]?.content?.parts?.[0]?.text ?? null;
    },
  },
  {
    id: 'groq', name: 'Llama 3.3 70B (Groq)', key: () => ENV.GROQ_API_KEY,
    call: async (prompt, system, key, signal) => {
      const url = 'https://api.groq.com/openai/v1/chat/completions';
      const messages = system ?[{ role: 'system', content: system }, { role: 'user', content: prompt }] :[{ role: 'user', content: prompt }];
      const payload = { model: 'llama-3.3-70b-versatile', messages, temperature: 0.2, max_tokens: 256 };
      const res = await aiFetchWithRetry(url, payload, { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, signal);
      return res.data?.choices?.[0]?.message?.content ?? null;
    },
  },
  {
    id: 'cerebras', name: 'Llama 3.3 70B (Cerebras)', key: () => ENV.CEREBRAS_API_KEY,
    call: async (prompt, system, key, signal) => {
      const url = 'https://api.cerebras.ai/v1/chat/completions';
      const messages = system ?[{ role: 'system', content: system }, { role: 'user', content: prompt }] :[{ role: 'user', content: prompt }];
      const payload = { model: 'llama-3.3-70b', messages, temperature: 0.2, max_tokens: 256 };
      const res = await aiFetchWithRetry(url, payload, { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, signal);
      return res.data?.choices?.[0]?.message?.content ?? null;
    },
  }
    ...
    try {
      const promises = this.active.map(p => 
        p.call(prompt, systemPrompt ?? '', p.key(), ac.signal).then(text => {
          ...
          log('INFO', 'AI', `응답 수신 완료 (${p.name})`);
          return { text, provider: p.name };
        }).catch(e => {
          // 🚀 쓰레기 로그 은닉: 다른 AI가 먼저 응답해서 강제 취소된 건이면 로그를 띄우지 않고 조용히 던짐
          if (!axios.isCancel(e) && e.name !== 'CanceledError' && e.code !== 'ERR_CANCELED') {
            log('WARN', 'AI', `${p.name} 실패`, e.message);
          }
          throw e;
        })
      );
      const result = await Promise.any(promises);
      ac.abort(); // 첫 응답 수신 즉시 나머지 대기중인 연결 강제 폭파 (메모리, 대역폭 방어)
      return result;
    } catch (err) {
      log('WARN', 'AI', '모든 AI Provider 응답 실패 (Timeout 또는 일시적 서버 장애)');
      return null;
    }
  }
...
/* ══════════════════════════════════════════════════════════════
   §5. Haversine & Cross-Source 전역 중복 방지망
══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   §6. Circuit Breaker & API 추적 (Error Shield)
══════════════════════════════════════════════════════════════ */
class CircuitBreaker {
  ...
  async exec(fn) {
    if (this.#state === 'OPEN') {
      ...
      log('CB', this.#name, '→ HALF_OPEN (복구 시험)');
    }
    try {
      const r = await fn();
      if (this.#failures > 0 || this.#state === 'HALF_OPEN') log('CB', this.#name, '→ CLOSED (정상 복구 완료)');
      this.#state = 'CLOSED'; this.#failures = 0;
      return r;
    } catch(e) {
      if (!e.cbOpen && ++this.#failures >= CFG.CB_THRESH) {
        this.#state = 'OPEN'; this.#openedAt = Date.now();
        log('WARN', this.#name, `→ OPEN (연쇄 에러 ${this.#failures}회로 인한 자동차단)`);
      }
      throw e;
    }
  }
  badge() {
    ...
    if (this.#state === 'HALF_OPEN') return '🟡 복구 대기';
...
/* ══════════════════════════════════════════════════════════════
   §9. Discord 클라이언트 (극한의 100MB 이하 메모리 다이어트)
══════════════════════════════════════════════════════════════ */
const discord = new Client({
  ...
  makeCache: Options.cacheWithLimits({
    ...
    MessageManager: 10,       // 메시지는 최소한만
    PresenceManager: 0,
    ThreadManager: 0,
    VoiceStateManager: 0,
    GuildMemberManager: 10,   // 대형 서버 멤버 캐싱 차단
    UserManager: 10,          // 유저 객체 캐싱 최소화
  }),
});

/* ══════════════════════════════════════════════════════════════
   §10. 브로드캐스트 큐 (Discord 429 Rate Limit 방지 Jitter 적용)
══════════════════════════════════════════════════════════════ */
  ...
  while(Q.length>0) {
    ...
    try {
      for (const id of getAlertChannels()) {
        ...
        // 🚀 채널 순회 시 Discord API 차단(429)을 피하기 위한 안전 지연(Jitter 추가)
        await sleep(CFG.BROADCAST_GAP + Math.random() * 100);
      }
      res();
    } catch(e){rej(e);}
  }
...
function aiField(ai) {
  if (!ai) return null;
  return { name: '🤖 AI 예측 (Gemini/Llama 3.3)', value: `**예상 최대여진:** M ${Number(ai.predictedMagnitude).toFixed(1)} (${sane(ai.aftershockProbability,100)})\n**💡 조언:** ${sane(ai.advice,500)}\n*(제공: ${sane(ai.provider,60)})*`, inline: false };
}
...
async function notifyErr(src, err) {
  ...
  c.msg = msg; c.at = now;
  await broadcast({embeds:[new EmbedBuilder().setTitle(`⚠️ [${src.toUpperCase()}] API 에러`).setColor(0xFF0000).addFields({name:'내용',value:sane(msg,512)},{name:'방어체계',value:'자동 재시도 및 우회 진행 중',inline:true}).setTimestamp()]}).catch(()=>{});
}
async function notifyRecover(src, n) {
  log('INFO', src, `정상 복구 완료 (${n}번째)`);
  await broadcast({embeds:[new EmbedBuilder().setTitle(`✅[${src.toUpperCase()}] 시스템 복구`).setColor(0x00FF99).setDescription(`API 연결이 정상화되었습니다. (${n}번째 시도)`).setTimestamp()]}).catch(()=>{});
}

/* ══════════════════════════════════════════════════════════════
   §12. 핵심 수집 엔진 (NDMS, KMA, JMA, USGS)
══════════════════════════════════════════════════════════════ */
...
async function fetchKMA() {
  ...
  try {
    const rows = await CB.kma.exec(() => withRetry(async () => {
      ...
      if (code && !CFG.KMA_OK.has(code)) throw new Error(`API Code Error: ${code}`);
...
async function fetchUSGS() {
  try {
    const features = await CB.usgs.exec(() => withRetry(async () => {
      // 🚀 USGS Time Sync 고도화: 서버 시간차이로 인한 미래 시간 요청(HTTP 400)을 막기 위해 2분 전부터 오버랩 조회 (중복은 자체 방어망이 차단)
      const after = usgsAfter ?? new Date(Date.now() - 15 * 60_000).toISOString();
      ...
      if (!data?.features) throw new Error('데이터 파싱 규격 불일치');
      usgsAfter = new Date(Date.now() - 2 * 60_000).toISOString(); 
    ...
    for (const f of features) {
      ...
      let embed;
      if (region==='KR') {
        embed = buildKR({title:'🌏 국내 인근 지진 (USGS)',footer:'USGS · 한반도 감지',loc:sane(p.place,256),mag,depth,intensity:null,time:timeMs,lat,lon,extra:[...(p.alert?[{name:'⚠️ PAGER',value:`${pagerMeta(p.alert).em} ${pagerMeta(p.alert).label}`,inline:true}]:[]),...extraFields]});
      } else if (region==='JP') {
        const placeKo = await toKo(p.place??'');
        embed = buildJP({title:p.tsunami===1?'🌊 쓰나미 경보 동반 (USGS)':'🌏 일본 인근 지진 (USGS)',footer:'USGS · 일본 감지',titleKo:placeKo,contentKo:null,mag,depth,lat,lon,isVolcano:false,isTsunami:p.tsunami===1,extra:extraFields});
      } else {
...
/* ══════════════════════════════════════════════════════════════
   §13. Anti-Raid 메모리 가비지 컬렉터 & 보안 로직
══════════════════════════════════════════════════════════════ */
const mentionLog = new Map();
const channelLog = new Map();

// 🚀 메모리 누수 원천 차단: 1분 주기로 만료된 Anti-Raid 캐시를 깔끔하게 소거하는 데몬
setInterval(() => {
  const now = Date.now();
  const mCut = now - (CFG.RAID_MENTION_SEC * 1000 * 2);
  for (const [k, v] of mentionLog) {
    const valid = v.filter(t => t > mCut);
    if (valid.length > 0) mentionLog.set(k, valid); else mentionLog.delete(k);
  }
  const cCut = now - (CFG.RAID_CH_SEC * 1000 * 2);
  for (const [k, v] of channelLog) {
    const valid = v.timestamps.filter(t => t > cCut);
    if (valid.length > 0) { v.timestamps = valid; channelLog.set(k, v); } else channelLog.delete(k);
  }
}, 60_000).unref();
...
async function handleEveryoneMention(msg) {
  ...
  if (times.length >= CFG.RAID_MENTION_LIMIT) {
    ...
    try {
      ...
      await sendAuditLog(msg.guild.id, buildAuditEmbed({actor:'System AI', target:`@${sane(msg.author.tag,100)}`, action:'was permanently banned (everyone spam)', before:`${times.length}회 / ${CFG.RAID_MENTION_SEC}초`, after:'영구 서버 차단', targetId:uid, color:0xFF0000}));
    } catch(e) {}
  }
}
async function handleChannelCreate(channel) {
  ...
  if (state.timestamps.length >= CFG.RAID_CH_LIMIT) {
    ...
    if (state.creatorId && state.creatorId !== discord.user?.id) {
      try { await channel.guild.members.ban(state.creatorId, { reason: '[자동] 채널 생성 폭주', deleteMessageSeconds: 604800 }); } catch(e){}
    }
    await sendAuditLog(channel.guild.id, buildAuditEmbed({actor:'System AI', target:state.creatorId?`<@${state.creatorId}>`:'알 수 없음', action:`channel flood detected — ${deleted.length}개 삭제`, before:`${state.timestamps.length}개 / ${CFG.RAID_CH_SEC}초`, after:`삭제 및 영구차단`, targetId:state.creatorId??'N/A', color:0xFF4500}));
  }
}
...
══════════════════════════════════════════════════════════════ */
const CMDS =[
  { name:'상태',   description:'봇 및 데이터 API 상태 모니터링' },
  { name:'마지막', description:'마지막 성공 조회 시각' },
  { name:'중복',   description:'Cross-Source 데이터 중복 억제율' },
  { name:'지역',   description:'USGS 지역 분류 체계 기준' },
  { name:'도움말', description:'시스템 가동 정보 안내' },
  { name:'청소',   description:'캐시 초기화 및 메모리 반환 (OWNER 전용)' },
...
discord.on(Events.InteractionCreate, async ix => {
  ...
  if (['청소','알림','로그'].includes(cmd) && !isOwner) return ix.reply({content:'❌ OWNER 전용 관리자 명령어입니다.',ephemeral:true});

  if (cmd === '알림' || cmd === '로그') {
    const chId = ix.options.getString('채널id','').trim();
    if (!/^\d{17,20}$/.test(chId)) return ix.reply({content:'❌ 올바른 형태의 채널 ID (17~20자리 숫자)를 입력하세요.',ephemeral:true});
    try {
      const ch = await discord.channels.fetch(chId); if (!ch?.isTextBased()) throw new Error('채팅 채널이 아님');
      const gid = ix.guildId??'global'; const cfg = GUILD_CFG.get(gid) ?? {alertChannel:null,logChannel:null};
      if (cmd === '알림') cfg.alertChannel = chId; else cfg.logChannel = chId;
      GUILD_CFG.set(gid,cfg); await persist('config');
      return ix.reply({content:`✅ 채널이 성공적으로 <#${chId}>에 바인딩되었습니다.`,ephemeral:true});
    } catch(e) { return ix.reply({content:`❌ 채널 권한 또는 조회 실패: ${sane(e.message,100)}`,ephemeral:true}); }
  }

  if (cmd === '상태') {
    const up = process.uptime(), anyErr = Object.values(TRK).some(t=>t.streak>0);
    return ix.reply({embeds:[new EmbedBuilder().setTitle('📊 시스템 상태 분석망 (v9.7 Ultimate Stable)').setColor(anyErr?0xFF6600:0x00FF99).addFields(
      {name:'🕐 연속 가동',value:`${Math.floor(up/3600)}h ${Math.floor((up%3600)/60)}m ${Math.floor(up%60)}s`,inline:false},
      ...
      {name:'🔄 중복 차단기',value:`${GEV.length}건 캐싱 (최대 ${CFG.DEDUP_MAX})`,inline:true},
      {name:'🤖 AI 신경망',value:aiManager.isAvailable?'✅ 최상위 병렬 활성화':'⚫ 미가동',inline:true}
    ).setTimestamp()]});
  }

  if (cmd === '마지막') return ix.reply({embeds:[new EmbedBuilder().setTitle('🕐 마지막 성공 동기화').setColor(0x5865F2).addFields({name:'📢 NDMS',value:TRK.ndms.lastOk?fmtDate(TRK.ndms.lastOk):'없음',inline:true},{name:'🌏 KMA',value:TRK.kma.lastOk?fmtDate(TRK.kma.lastOk):'없음',inline:true},{name:'🗾 JMA',value:TRK.jma.lastOk?fmtDate(TRK.jma.lastOk):'없음',inline:true},{name:'🌐 USGS',value:TRK.usgs.lastOk?fmtDate(TRK.usgs.lastOk):'없음',inline:true}).setTimestamp()]});
  if (cmd === '중복') {
    const bySrc={}; for (const ev of GEV) bySrc[ev.src]=(bySrc[ev.src]??0)+1;
    return ix.reply({embeds:[new EmbedBuilder().setTitle('🔄 전역 중복 식별 방어망').setColor(0x5865F2).addFields({name:'현재 레코드',value:`${GEV.length} / ${CFG.DEDUP_MAX}`,inline:true},{name:'소스 분류',value:Object.entries(bySrc).map(([s,n])=>`${s}: ${n}`).join('\n')||'데이터 없음',inline:true},{name:'중복 격리 기준',value:`거리 ≤${CFG.DEDUP_DIST_KM}km · 규모오차 ±${CFG.DEDUP_MAG_D} · 발생시각차 ±${CFG.DEDUP_TIME_MS/60_000}분`,inline:false}).setTimestamp()]});
  }
  if (cmd === '지역') return ix.reply({embeds:[new EmbedBuilder().setTitle('🗺️ USGS 글로벌 분류 규격').setColor(0x5865F2).addFields({name:'🇰🇷 대한민국 (KR)',value:`위도 33.0°~38.9° / 경도 124.5°~132.0°`,inline:false},{name:'🇯🇵 일본 (JP)',value:`위도 24.0°~46.0° / 경도 122.0°~154.0°`,inline:false},{name:'🌐 인터내셔널 (INT)',value:'범위 외 전역',inline:false}).setTimestamp()]});
  ...
  if (cmd === '청소') {
    ...
    return ix.reply({content:`🧹 모든 임시 메모리 및 캐시가 강제 반환/소거되었습니다.`, ephemeral:true});
  }

  if (cmd === '도움말') return ix.reply({embeds:[new EmbedBuilder().setTitle('📖 봇 v9.7 도움말').setColor(0x5865F2).setDescription('국내외 지진·화산·재난문자 통합 실시간 알림 시스템 (AI 예측 엔진 탑재)').addFields({name:'📢 수집 소스', value:'NDMS(2분) / KMA(5분) / JMA(5분, 자동번역) / USGS(5분, M4.5+ 분류)'},{name:'🤖 최상급 AI 신경망 (M5.0+)', value:'Gemini 2.0 Flash / Llama 3.3 70B 병렬 구성\n*초저지연 Promise.any 레이싱 모델 탑재*'},{name:'🛡️ 강력 보안/최적화', value:'도배방지 GC 데몬, 메모리 제한 Bounded 캐시, 네트워크 이중 백오프'},{name:'⚙️ 관리자', value:'`/알림` `/로그` `/청소`'}).setTimestamp()]});
});

/* ══════════════════════════════════════════════════════════════
   §15. 웹 서버 방어막 (Express) & 구동 루프
══════════════════════════════════════════════════════════════ */
const app = express();
const server = app.listen(CFG.PORT, () => log('INFO', 'WEB', `보안망 구성 완료 - 포트 ${CFG.PORT}`));

app.use(helmet({ crossOriginEmbedderPolicy: false }));
app.use(rateLimit({ windowMs: 15*60_000, max: 100, standardHeaders: true, legacyHeaders: false }));
app.use(express.json({ limit: '10kb' }));

app.get('/health', (_, res) => res.status(Object.values(CB).some(c=>c.isOpen)?503:200).json({ status: 'ok', uptime: process.uptime(), memoryMB: (process.memoryUsage().rss / 1024 / 1024).toFixed(2) }));
app.use((_,res)=>res.status(404).json({error:'Not Found'}));

discord.once(Events.ClientReady, async () => {
  log('INFO', 'BOT', `접속 성공: ${discord.user.tag}`);
  await initStorage();
...
/* ══════════════════════════════════════════════════════════════
   §17. 무중단 격리 체계 (Node.js Crash Preventer)
══════════════════════════════════════════════════════════════ */
// 🚀 봇 다운을 유발하는 치명적 Uncaught 에러들을 잡아내어 단순히 로그만 띄우고 무중단 서비스 진행
process.on('uncaughtException', e => log('CRITICAL', 'SYS', 'Uncaught Exception', e.message));
process.on('unhandledRejection', e => log('CRITICAL', 'SYS', 'Unhandled Rejection', e?.message||String(e)));

async function graceful() {
  log('INFO', 'SYS', '정상 종료 프로세스 개시');
